...
### Lab 2: The Interactive TaskFlow Dashboard
**Goal:** Build a robust task manager that dynamically creates, checks off, and deletes tasks utilizing event delegation and safe DOM manipulation.

**Step-by-Step Instructions:**
1. Build an HTML structure featuring an `<input id="task-input">`, a `<button id="add-btn">Add</button>`, and an `<ul id="todo-list"></ul>`.
2. Listen for clicks on the add button (or 'Enter' presses on the input).
3. Grab the value from the input. If it's empty, `return` early.
4. Programmatically construct a new `<li>`. Inside the `<li>`, construct a `<span>` holding the task text, and a `<button class="delete-btn">X</button>`. Append the span and button to the `<li>`.
5. Append the fully constructed `<li>` to the `<ul>`. Clear the input field.
6. Attach exactly **one** click event listener to the `#todo-list` container (Delegation).
7. If the `e.target` is the delete button (or an icon inside it), locate the parent `<li>` and call `.remove()` on it.
8. If the `e.target` is the span text, toggle a CSS class `.completed` on the `<li>` that applies a text strikethrough.
