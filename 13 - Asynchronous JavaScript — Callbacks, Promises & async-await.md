...
### Lab 1: Fetch & Render User Profiles (45 min)

**Goal:** Fetch data from a real API and render it to the DOM.

1. Open `labs/lab1-fetch-profiles/`.
2. Write an `async` function `loadProfiles()` that:
   - Fetches `https://jsonplaceholder.typicode.com/users`
   - Fetches `https://jsonplaceholder.typicode.com/posts` **at the same time** using `Promise.all`
   - For each user, counts how many posts they have
   - Renders a card for each user: name, email, post count
3. Show a loading spinner while fetching, hide it in `finally`.
4. Show an error message if the fetch fails.

### Lab 2: Abortable Live Search (45 min)

**Goal:** Implement a live search that cancels stale requests.

1. Open `labs/lab2-abort-search/`.
2. Create a search input that fetches from `https://jsonplaceholder.typicode.com/todos?q={query}` on every keystroke.
3. Use `AbortController` to cancel any ongoing request when a new keystroke arrives.
4. Add a 300ms debounce (delay before firing the search) to reduce unnecessary requests.

```js
// Starter code for debouncing
...
searchInput.addEventListener('input', (e) => {
  ...
  debounceTimer = setTimeout(() => {
    search(e.target.value); // Only fires 300ms after last keystroke
  }, 300);
});
```
...
Integrate a real API into TaskFlow!

### Requirements

**1. Add a sync function to `taskModel.js`:**

```js
// Simulate saving to a server (JSONPlaceholder accepts POST but doesn't really save)
export async function syncTasksWithServer(tasks) {
...
**2. Update `app.js` to call this after every state change:**

```js
async function saveAndRender() {
  render(); // Update UI immediately (optimistic update)

  const statusEl = document.querySelector('#sync-status');
  statusEl.textContent = "Saving...";

  try {
    await syncTasksWithServer(getTasks());
...
**3. Handle errors gracefully in the UI** — Show a red error banner if sync fails, and a green "Saved!" message on success.
...
- **ES2024 `Promise.withResolvers()`** cleanly decouples promise creation from resolution.
- **`async`/`await`** makes async code read like synchronous code — use `try`/`catch` for errors.
- **`fetch()`** does NOT throw on HTTP errors — always check `response.ok`.
- Use **`AbortController`** to cancel fetch requests and prevent stale data from overwriting fresh data.
