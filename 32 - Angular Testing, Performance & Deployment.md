...
2. Vercel automatically detects Angular projects. Ensure the Output Directory is set to `dist/<app-name>/browser`.
3. Vercel automatically configures SPA routing rules by default.
...
beforeEach(async () => {
  ...
  component = fixture.componentInstance;
  fixture.detectChanges(); // Trigger initial template render cycle
});

// Setting a signal input value safely within a test environment
fixture.componentRef.setInput('propertyName', newValue);

// Querying the DOM and simulating user interactions
const el = fixture.nativeElement.querySelector('[data-testid="submit-btn"]');
el.click();
fixture.detectChanges(); // 🚨 MUST explicitly call after DOM interaction to reflect changes
```

### HttpTestingController Operations
```typescript
let httpMock: HttpTestingController;

// Inside an 'it' block asserting a network call:
const req = httpMock.expectOne('/api/data/endpoint');
expect(req.request.method).toBe('POST'); // Validate the request type

// Resolve the request with a successful mock payload synchronously
req.flush({ id: 1, status: 'success' }); 

// Alternatively, resolve the request simulating a critical server failure
// req.flush('Internal Error', { status: 500, statusText: 'Server Error' });

// Ensure rigorous cleanup in afterEach block to prevent flaky tests
// afterEach(() => httpMock.verify());
```

### @defer Block Implementations
```html
@defer (on viewport) {
  <app-heavy-lazy-component />
} 
@placeholder {
  <div class="skeleton">Visible instantly before trigger event occurs</div>
} 
@loading (minimum 500ms) {
  <div class="spinner">Visible during active network download (min delay prevents flash)</div>
} 
@error {
  <div class="error">Fallback UI presented if the network request drops or fails</div>
}
```
