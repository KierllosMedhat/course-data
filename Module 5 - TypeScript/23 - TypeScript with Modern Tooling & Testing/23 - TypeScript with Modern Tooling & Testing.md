# Lecture 23 — TypeScript: Modern Tooling, ESLint Flat Config & Vitest
...
Browsers and Node.js don't understand TypeScript. A **build tool** converts `.ts` files into plain JavaScript. Different tools exist for different use cases:
...
**Vite** is the modern choice for building web apps. It uses **esbuild** (written in Go) to be 10–100x faster than traditional compilers.

**Key fact:** Vite transpiles code quickly but does **not** perform type checking. Run `tsc --noEmit` separately.
...
Tests are like load tests for a bridge—they prove each component works correctly before assembly.
They also enable **fearless refactoring**: tests immediately catch breakages when you change existing code.
...
A **linter** analyses code for bugs, style issues, and bad practices before runtime.

### ESLint 9 — The Flat Config System

ESLint 9 introduced the **Flat Config** system, replacing old `.eslintrc` files with a single `eslint.config.js`. It uses a simple **array of configuration objects** for better clarity.
...
A team can adopt linters and tests, but developers may skip them under pressure. Pre-commit hooks solve this by **blocking commits that fail quality checks**.
...
When a value has a **union type** (e.g., `string | number`), TypeScript doesn't know its specific type. **Type narrowing** uses checks to determine the specific type at runtime.
...
## 5. 💡 Think Like a Dev
Professional developers prioritize **automation over discipline**. They configure Git hooks to force tests, and tools to standardize code style. Tooling removes cognitive load so you can focus on building features.
...
- **Discriminated unions** are the #1 TypeScript pattern for modelling state.
- **Mapped types** transform properties; **conditional types** provide type-level logic.

### 📚 Extensive Tutorials & Resources
- **Vitest Docs:** [Getting Started with the Vitest Testing Framework](https://vitest.dev/guide/)
- **ESLint Docs:** [Configuration Files (Flat Config) Guide](https://eslint.org/docs/latest/use/configure/configuration-files)
- **TypeScript Handbook:** [Type Narrowing - Type Guards and Type Predicates](https://www.typescriptlang.org/docs/handbook/2/narrowing.html)
- **TypeScript Handbook:** [Mapped Types - Creating Types from Types](https://www.typescriptlang.org/docs/handbook/2/mapped-types.html)
- **TypeScript Handbook:** [Conditional Types and Type Inference](https://www.typescriptlang.org/docs/handbook/2/conditional-types.html)
- **Vitest Docs:** [Mocking Guide and Mocking APIs](https://vitest.dev/guide/mocking.html)
- **Vite Guide:** [Vite Features - TypeScript Support](https://vite.dev/guide/features.html#typescript)

---

**Next Lecture:** [Lecture 24 — Angular Architecture & First Application](../../Module%206%20-%20Angular%20Framework/24%20-%20Angular%20Architecture%20%26%20First%20Application/24%20-%20Angular%20Architecture%20%26%20First%20Application.md) — Module 5 begins!
