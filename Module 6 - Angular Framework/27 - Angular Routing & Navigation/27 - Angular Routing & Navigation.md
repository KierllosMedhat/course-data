# Lecture 27 — Angular Routing & Navigation
...
In traditional Multi-Page Applications (MPAs), every link click causes a full page reload. Single Page Applications (SPAs) solve this by loading ONE HTML file once. When users navigate, Angular intercepts the click, updates the URL, and dynamically swaps out the UI without reloading.

**How Angular's Router Works:**
1. User clicks `<a routerLink="/products">`.
2. Router intercepts click and updates URL.
3. Router matches `/products` to `ProductListComponent` in `app.routes.ts`.
4. `<router-outlet>` renders the new component instantly.
...
Use `routerLink` instead of `href` for all in-app navigation to prevent full page reloads.

```html
<!-- Navigate without reloading -->
<a routerLink="/products">Products</a>

<!-- Dynamic link with properties -->
...
Use `Router` to navigate in response to logic (e.g., after login).
...
Child routes let you build **nested layouts** where a parent component provides shared UI (like a sidebar) and the child routes swap out just a portion of the page inside the parent's `<router-outlet>`.
...
export const routes: Routes = [
  {
    path: 'dashboard',
    component: DashboardComponent, // MUST have its own <router-outlet>
    children: [
...
The parent component (`DashboardComponent`) uses `<router-outlet>` to render its children:

```ts
@Component({
  template: `
    <div class="dashboard-layout">
      <aside><a routerLink="overview">Overview</a></aside>
      <main><router-outlet /></main> <!-- Child renders here -->
    </div>
  `
})
...
- **Angular.dev:** [Angular Routing Overview & Setup](https://angular.dev/guide/routing/common-router-tasks)
- **Angular University:** [Angular Router: The Complete Guide](https://blog.angular-university.io/angular-router/)
- **Angular.dev:** [Functional Route Guards in Angular](https://angular.dev/guide/routing/router-guards)
- **Angular University:** [Angular Component Inputs for Route Parameters](https://blog.angular-university.io/angular-component-input-binding/)
- **FreeCodeCamp:** [How to Set Up Routing in an Angular Application](https://www.freecodecamp.org/news/angular-routing-tutorial/)
- **Angular University:** [Angular Lazy Loading: Complete Guide](https://blog.angular-university.io/angular-route-lazy-loading/)
- **Fireship (YouTube):** [Angular Routing & Lazy Loading Explained](https://www.youtube.com/watch?v=23S0Z306Yq8)

**Next Lecture:** [Lecture 28 — Angular Services & Dependency Injection](../28%20-%20Angular%20Services%20%26%20Dependency%20Injection/28%20-%20Angular%20Services%20%26%20Dependency%20Injection.md)