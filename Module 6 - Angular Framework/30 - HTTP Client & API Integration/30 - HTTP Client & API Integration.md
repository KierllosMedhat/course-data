# Lecture 30 — HTTP Client & API Integration
...
### 3.1. The Big Picture: What is an API and HTTP?
...
### 3.2. `HttpClient` Basics in Modern Angular
...
### 3.4. Request Configuration: Headers & Params
...
### 3.5. Functional Interceptors: Auth Tokens & Middleware
...
### 3.6. Resilience: Error Handling Patterns & Retries
...
### 3.7. The Modern Way: `rxResource` and `resource`
...
### 3.8. Security & Browser Mechanisms: CORS and Preflight
...
### Final Assignment: E-commerce Storefront HTTP Integration (90 mins)
**Context:** We aren't going to wait for our C# backend to be built! In the real world, frontend developers often build features independently using third-party APIs or mock servers.
**Scenario:** Take your existing ShopAngular e-commerce project and migrate it from static mock data to a live API. We will build a realistic "E-commerce Storefront" using the FakeStoreAPI, making real HTTP requests.

**Requirements:**
1. **API Integration:** Utilize the Fake Store API (`https://fakestoreapi.com/products`).
2. **Type Safety:** Implement full typed interfaces for `Product` (`id`, `title`, `price`, `description`, `category`, `image`, `rating`).
3. **Service Layer:** Replace the local data array in your `ProductService` with real `HttpClient` calls. Implement `getAllProducts(): Observable<Product[]>` and `getProductById(id: number): Observable<Product>`.
4. **Reactive UI:** Refactor the `ProductListComponent` and `ProductDetailComponent` to utilize modern Angular 19 `rxResource`.
5. **State Management:** Ensure a beautiful loading spinner or skeleton loader is visible while the products are being fetched. Implement a clean error state UI with a "Try Again" reload button, effectively handling network failures.
6. **Bonus (Optional):** Use `HttpParams` to add a category filter dropdown that fetches products by specific categories (`https://fakestoreapi.com/products/category/jewelery`).
...
**Next Lecture:** [Lecture 31 — Angular Material & Component Libraries](../31%20-%20Angular%20Material%20%26%20Component%20Libraries/31%20-%20Angular%20Material%20%26%20Component%20Libraries.md)

### 📚 Extensive Tutorials & Resources
- **Source:** [Angular University - Angular HttpClient: The Complete Guide](https://blog.angular-university.io/angular-http/)
- **Source:** [Angular.dev - Interacting with Data from a Server Guide](https://angular.dev/guide/http)
- **Source:** [FreeCodeCamp - How to Integrate APIs in Angular using HttpClient](https://www.freecodecamp.org/news/how-to-make-http-requests-in-angular/)
- **Source:** [Angular.dev - Functional Interceptors API Reference](https://angular.dev/api/common/http/HttpInterceptorFn)
- **Source:** [Angular University - Introduction to Angular Signals and rxResource](https://blog.angular-university.io/angular-signals/)
- **Source:** [Web.dev - Understanding CORS and HTTP Access Control](https://web.dev/articles/cross-origin-resource-sharing)
